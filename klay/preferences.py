...
from klay.errors.friendly_error import FriendlyError
...
from klay.importer.location import UnresolvableLocationError
...
from klay.utils.create_dialog import create_dialog


@Gtk.Template(resource_path=shared.PREFIX + "/gtk/preferences.ui")
class KlayPreferences(Adw.PreferencesDialog):
    __gtype_name__ = "KlayPreferences"
    ...
    def reset_app(self, *_args: Any) -> None:
        rmtree(shared.data_dir / shared.DATA_DIR_NAME, True)
        rmtree(shared.config_dir / shared.DATA_DIR_NAME, True)
        rmtree(shared.cache_dir / shared.DATA_DIR_NAME, True)
